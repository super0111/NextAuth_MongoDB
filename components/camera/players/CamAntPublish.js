...
export default function CamAntPublish(props) {
    const { stream_id } = props
    const [streamName, setStreamName] = useState(stream_id)
    ...
    useEffect(() => {
        let videox = document.querySelector(`#localVideo${stream_id}`);
...
//     }
//     console.log('Connected-ClientSide Publishing')
// }, [webRTCAdaptor,connected])
    ...
    const  initiateWebrtc = () =>{
        return new WebRTCAdaptor({
            ...
            localVideoId: `localVideo${stream_id}`,
            debug: true,
            bandwidth:900,
    ...
    return (
        <div>
           ...
           <div className="max-w-[800px] max-h-[500px]">
                <video width="100%" height="auto" id={`localVideo${stream_id}`} autoPlay muted  playsInline/>
            </div>
